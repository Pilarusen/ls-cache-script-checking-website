# === BLOKADA WSZYSTKICH BOTÓW ===
<IfModule mod_rewrite.c>
RewriteEngine On
RewriteCond %{HTTP_USER_AGENT} (Googlebot|Googlebot-Image|Googlebot-News|Googlebot-Video|AdsBot-Google|Mediapartners-Google|facebookexternalhit|meta-externalagent|Facebot|Amazonbot|MJ12bot|VeBot|BLP_bbot|AhrefsBot|datagnionbot|YandexBot|PetalBot|YandexImages|bingbot|SemrushBot|AspiegelBot|dotbot|Baiduspider|ClaudeBot|HTTrack|Wget|WebCopier|WebZIP|Teleport|WebStripper|WebReaper|WebSauger|EmailSiphon|EmailCollector|EmailWolf|Offline.*Explorer|Offline.*Navigator|GetRight|GetWeb|Go!Zilla|FlashGet|Download.*Demon|Download.*Wonder|NetAnts|Mass.*Downloader|larbin|ia_archiver|Xenu|Zeus|Anarchie|ASPSeek|attach|autoemailspider|BackWeb|Bandit|BatchFTP|BlackWidow|Bot.*mailto|Buddy|bumblebee|CherryPicker|ChinaClaw|Collector|Copier|Crescent|Custo|DIIbot|DISCo|Downloader|Drip|eCatch|EirGrabber|Express.*WebPictures|ExtractorPro|EyeNetIE|FileHound|FrontPage|gigabaz|Grabber|GrabNet|Grafula|grub-client|HMView|httpdown|Image.*Stripper|Image.*Sucker|Indy.*Library|InterGET|Internet.*Ninja|InternetSeer|Iria|JetCar|JOC.*Web.*Spider|JustView|LeechFTP|LexiBot|lftp|likse|LinkWalker|Mag-Net|Magnet|Memo|Mirror|Mister.*PiX|MSIECrawler|MSProxy|Navroad|NearSite|NetMechanic|NetSpider|Net.*Vampire|NetZIP|NICErsPRO|Ninja|Octopus|Openfind|PageGrabber|Papa.*Foto|pavuk|pcBrowser|psbot|Pump|QRVA|RealDownload|Reaper|Recorder|ReGet|Scooter|Seeker|Siphon|SiteSnagger|SlySearch|SmartDownload|Snake|SpaceBison|sproose|Stripper|Sucker|SuperBot|SuperHTTP|Surfbot|Szukacz|tAkeOut|URLSpiderPro|Vacuum|VoidEYE|Web.*Image.*Collector|Web.*Sucker|WebAuto|WebBandit|webcollage|Web.*Downloader|WebEMailExtrac|WebFetch|WebGo.*IS|WebHook|WebLeacher|WebMiner|WebMirror|Website.*eXtractor|Website.*Quester|Webster|WebWhacker|Whacker|Widow|WWWOFFLE|x-Tractor|Xaldon.*WebSpider|claudebot@anthropic\.com) [NC]
RewriteRule .* - [R=403,L]
</IfModule>
# === /BLOKADA BOTÓW ===
